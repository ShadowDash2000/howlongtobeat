...
func Test_prepSearchRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := mockClient.prepSearchRequest(tt.term, tt.searchModifier, tt.pagination, TokenResponse{}); !reflect.DeepEqual(got, tt.want) {
...
func Test_searchHTTPRequest(t *testing.T) {
	...
	req, err := mockClient.searchHTTPRequest(context.Background(), body, "", TokenResponse{})
	if err != nil {
	...
	if strings.HasPrefix(req.URL.String(), "/api/search") {
		t.Errorf("searchHTTPRequest() url = %v, want prefix %v", req.URL.String(), "/api/search")
	}
...
func Test_Search(t *testing.T) {
	...
	result, err := mockClient.Search(ctx, "The Witcher 3 Wild Hunt", SearchModifierNone, nil)
	if err != nil {
...
func Test_Search_NullSearchTerm(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("Search() expected error, but received: %v", err)
	}

	if !errors.Is(err, EmptySearchTermErr) {
		t.Fatalf(`Search() expected %v" error, but received: %v`, EmptySearchTermErr, err)
	}
}
